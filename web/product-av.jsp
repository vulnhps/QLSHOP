...
<html>
    ...
    <body>
        <%
            ProductDAO aa = new ProductDAO();
            String ll = "";
            if (request.getParameter("category") != null) {
                ll = request.getParameter("category");
            }
        %>
                ...
                <div class="hot-products">
                    ...
                    <ul id="hot">
                        <form action="controllerCart" >
                            <div class="row"> 
                                <%                                   
                                    for (Product p : aa.getListProductByCategory(Long.parseLong(ll))) {
                                %>                                       
                                <div class="col-md-3 col-sm-6">
                                    <div class="products">
                                        <div class="offer">Hot</div>
                                        <div class="thumbnail"><a href="single-av.jsp?product_id=<%=p.getProduct_id()%>"><img src="images/<%=p.getProduct_img()%>" alt="<%=p.getProduct_name()%>"></a></div>
                                        <div class="productname"><%=p.getProduct_name()%></div>
                                        <h4 class="price">$<%=p.getProduct_price()%></h4>
                                        <div class="button_group">
                                            <button class="button add-cart">
                                                <a href="controllerCa?command=plus&product_id=<%=p.getProduct_id()%>">Thêm vào giỏ hàng</a>
                                            </button>
                                        </div>
                                    </div>              
                                </div>
